...
const HeaderLinkContent: React.FC<{ item: any }> = ({ item }) => {
  ...
  return (
    <li>
      <Link
        href={item.href}
        className={`px-4 py-2 font-medium transition-all duration-500 ease-in-out hover:text-black dark:hover:text-black hover:bg-white/80 dark:hover:bg-white/15 hover:rounded-3xl hover:shadow-lg hover:scale-105
                    ${
                      activeLink === item.href
                        ? 'bg-white dark:bg-white/20 text-black dark:text-white rounded-[90rem] shadow-lg scale-105'
                        : 'text-dark_black/60 dark:text-white/70'
                    }`}>
        {item.label}
      </Link>
    </li>
