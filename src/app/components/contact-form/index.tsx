  ...
  return (
    <div className='relative' ref={ref}>
      <button
        ...
        type='button'
        onClick={() => setOpen((s) => !s)}
        className='w-full mt-2 flex items-center justify-between rounded-full border px-5 py-3 text-left transition dark:border-white/20 focus:outline-hidden dark:bg-black/40 hover:border-dark_black/50 dark:hover:border-white/40'>
        <span className={`${value ? '' : 'text-dark_black/50 dark:text-white/50'}`}>
        ...
        <Icon
          ...
          width='22'
          height='22'
          className='text-dark_black/60 dark:text-white/60'
        />
      </button>
      ...
      <div
        role='listbox'
        aria-labelledby={id}
        className={`absolute z-20 mt-2 w-full origin-top overflow-hidden rounded-2xl border border-white/20 bg-white/80 dark:bg-black/70 backdrop-blur-md shadow-xl transition-all duration-200 ${
      ...
      >
        {options.map((opt) => (
          <button
            ...
            className={`w-full text-left px-4 py-2.5 transition-colors duration-150 hover:bg-dark_black/5 dark:hover:bg-white/10 ${
              value === opt.value ? 'bg-dark_black/5 dark:bg-white/10' : ''
            }`}
          >
            {opt.label}
          </button>
